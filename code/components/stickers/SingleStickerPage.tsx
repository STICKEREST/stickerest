...
export const SingleSticker = () => {
  ...
  const [downloading, setDownloading] = React.useState<number>(0);

  const finishedDownloading = () => {
    setDownloading(value => value-1);
  }
  ...
  const loadStickerPage = () => {
    ...
    fetch(`https://stickerest.herokuapp.com/stickers/${id}`)
      .then((result) => result.json())
      .then((result) => {setSticker(result[0]); finishedDownloading()});
    fetch(`https://stickerest.herokuapp.com/stickers/images-${id}`)
      .then((result) => result.json())
      .then((result) => {setStickerImage(result.slice(1)); finishedDownloading();});
  }
